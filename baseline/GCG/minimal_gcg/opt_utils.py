...
from baseline.GCG import get_embedding_matrix, get_embeddings
...
def greedy_sample_control(
    ...
    new_token_val = top_indices[new_token_pos, :1]
    new_control_toks = original_control_toks.scatter_(
        1, new_token_pos.unsqueeze(-1), new_token_val
    )
...
def prob_sample_control(
    ...
    # Convert gradients to probabilities (softmax)
    probs = torch.softmax(-grad / temp, dim=1)
    top_probs, top_indices = torch.topk(probs, topk, dim=1)
    # Sample based on the probabilities
    new_token_val = torch.multinomial(top_probs[new_token_pos], 1)
    new_control_toks = original_control_toks.scatter_(
        1, new_token_pos.unsqueeze(-1), new_token_val
    )
...
def get_logits(
    *, model, tokenizer, input_ids, control_slice, test_controls=None, return_ids=False
):
    ...
    if any(model_name in model_path for model_name in model_names):
        model = (
            AutoModelForCausalLM.from_pretrained(
                model_path, load_in_8bit=True, torch_dtype=torch.bfloat16, **kwargs
            )
            .to(device)
            .eval()
        )
